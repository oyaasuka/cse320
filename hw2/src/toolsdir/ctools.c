...

{
  ...
  while (T) {
    ...
    switch (len = rolo_getline(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int rolo_getline (iport,buffer,buflen) FILE *iport; char *buffer; int buflen;
...

{
  int len;
  char *line;
  if (0 > (len = rolo_getline(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
...

{
  ...
  while (T) {
    ...
    if (0 > (len = rolo_getline(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
